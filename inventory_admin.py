...
app.title('Inventory Management (admin)')
...
def insert():
    ...
    sale_price= sale_price_entry.get()
    today_date = f'{date.today():%y %m %d}'
    if not (id and name and stock and price and sale_price):
        messagebox.showerror('Error','Enter all feilds.')
        ...
        try:
            ...
            database.insert_product(id,name,catogry,stock_value,price_value,sale_price_value,today_date)
            add_to_treeview()
            clear()
...
frame = customtkinter.CTkFrame(app,bg_color='black',fg_color='purple',corner_radius=10,border_width=2,border_color='black',width=200,height=650)
...
add_button = customtkinter.CTkButton(frame,command=insert,font=font2,text_color='purple',text='Add',fg_color='black',hover_color='plum',bg_color='black',cursor='hand2',corner_radius=8,width=80)
add_button.place(x=15,y=500)
...
clear_button.place(x=108,y=500)
...
update_button.place(x=15,y=540)
...
delete_button.place(x=108,y=540)
...
tree = ttk.Treeview(app,height=35)
